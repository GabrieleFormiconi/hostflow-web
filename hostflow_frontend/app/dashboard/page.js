...
const DEFAULT_SETTINGS = {
  ...
  cleaningHours: "0",
  cityTax: false,
  cityTaxMode: "Aggiungi come incasso cash",
  cityTaxRate: "4",
...
function enrichRow(row, settings, allRows, cleaningServices = []) {
  ...
  const cityTaxMode = settings.cityTaxMode || (settings.cityTax ? "Sottrai dal netto" : "Non considerare");
  const calculatedCityTax = cancelled || cityTaxMode === "Non considerare" ? 0 : toNum(row.guests || 1) * n * toNum(settings.cityTaxRate);
  ...
  const withholdingBase = cityTaxMode === "Sottrai dal netto" ? Math.max(0, total - calculatedCityTax) : total;
  const withholding = cancelled || !settings.withholding ? 0 : (importedWithholding > 0 ? importedWithholding : withholdingBase * toNum(settings.withholdingPct) / 100);
  const cityTaxImpact = cityTaxMode === "Sottrai dal netto" ? -calculatedCityTax : cityTaxMode === "Aggiungi come incasso cash" ? calculatedCityTax : 0;
  const netOperating = cancelled ? 0 : total - cleaning - platformFee - trx - vat + cityTaxImpact;
  const netReal = netOperating - withholding;
  return { ...row, nights: n, total_price: total, cleaning_cost: cleaning, platform_fee: platformFee, transaction_cost: trx, city_tax: calculatedCityTax, vat_platform_services: vat, cleaning_allocated: cleaning, withholding_tax: withholding, net_operating: netOperating, net_real: netReal, adr: n ? total / n : 0 };
}
