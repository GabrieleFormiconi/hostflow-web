...
function enrichRow(row, settings, allRows, cleaningServices = []) {
  ...
  const platformFee = cancelled || isCustom ? 0 : (importedPlatformFee > 0 ? importedPlatformFee : total * toNum(settings.commissionFallbackPct || 15) / 100);
  ...
  const cityTaxMode = settings.cityTaxMode || (settings.cityTax ? "subtract" : "ignore");
  const cityTax = cancelled || cityTaxMode === "ignore" ? 0 : toNum(row.guests || 1) * n * toNum(settings.cityTaxRate);
  ...
  const cityTaxNetImpact = cityTaxMode === "cash" ? cityTax : cityTaxMode === "subtract" ? -cityTax : 0;
  const netOperating = cancelled ? 0 : total - cleaning - platformFee - trx - vat + cityTaxNetImpact;
  const withholding = cancelled || !settings.withholding ? 0 : total * toNum(settings.withholdingPct) / 100;
  const netReal = netOperating - withholding;
  return { ...row, nights: n, total_price: total, cleaning_cost: cleaning, platform_fee: platformFee, transaction_cost: trx, city_tax: cityTax, vat_platform_services: vat, cleaning_allocated: cleaning, withholding_tax: withholding, net_operating: netOperating, net_real: netReal, adr: n ? total / n : 0 };
}
